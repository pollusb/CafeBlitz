# after a new First Responder Kit revision is made availabe
# clone repo in frk, but keep only relevent stuff
# keep the old version in frk.old to compare
...
Get-ChildItem $PSScriptRoot\frk -Exclude sp_Blitz*.sql -Include .git -Recurse | Remove-Item -Recurse -Confirm:$false
Get-ChildItem .git -Recurse -Attributes H | Remove-Item -Force -Confirm:$false
