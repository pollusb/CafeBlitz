function Invoke-BlitzFirst {
    ...
    param (
        ...
        $Path = "$home\Code\Temp" # $Path\Machine\Instance\First\$SortOrder
    )
    ...
    foreach ($sql in $SqlInstance) {
        ...
        mkdir "$Path\$($sql.ToUpper())\First" -ea SilentlyContinue | Select-Object Fullname | Out-Verbose

        $fileClixml = '{0}\{1}\First\blitz.first.xml' -f $Path, $sql
