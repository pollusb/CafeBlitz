...
if (($SkipTest -lt 1 -and $SkipTest -ne 0) -or $FilterTest -contains 1) {
    Write-Host "01. Testing the simple call with no param" @header
    $obj = Invoke-CafeBlitz -SqlInstance $SqlInstance -Verbose:$Verbose
...
if (($SkipTest -lt 2 -and $SkipTest -ne 0) -or $FilterTest -contains 2) {
    ...
    $obj = Invoke-CafeBlitz -SqlInstance $SqlInstance -Verbose:$Verbose -OutputProcedureCache
    Write-Host ('{0} lines in result' -f $obj.Result.Count) @info
}

if (($SkipTest -lt 3 -and $SkipTest -ne 0) -or $FilterTest -contains 3) {
    Write-Host "03. Testing with XML output" @header
    $obj = Invoke-CafeBlitz -SqlInstance $SqlInstance -Verbose:$Verbose -OutputType XML
    Write-Host ('{0} lines in result' -f $obj.Result.Count) @info
}
if ($PassThru) { $obj }